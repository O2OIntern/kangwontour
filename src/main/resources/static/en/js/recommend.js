...
/**
 * reco_detail_info에서 코스 시작위치로 가는 Tmap 버튼을 생성하기 위한 선택된 코스의 시작좌표를 저장하는 변수
 * 현재 사용 안함
 */
var selectedcourse = {
	"mapx": "0",
...
/**
 * "추천 코스" intent에 사용할 함수
 * @param data.command : action.js에서 받은 커맨드
 */
 ...
 * @param {*} data fulfillment에서 가져온 데이터
 */
const reco_step_two_view = async (data) => {
	...
	if (fallback) {
		...
		for (let i = 0; i < ITEM.length; i++){
			if (ITEM[i].category === "음식점") {
				if (!ITEM[i].mapx) {
					console.log(`${i} : 좌표 데이터 없음`);
				}
				else intermediaterest.push(ITEM[i]);
			}
		}
		...
		for (let i = 0; i < 18; i++) {
			...
			intermediaterest.forEach(data => {
				if(data.sigungucode == i + 1) {
					restidreco[i].push(data);
				}
			})
		}
	}
...
const reco_step_result_view = async (data) => {
	...
	for (let i = 0; i < 18; i++){
		let selector = random_number(0, resultidreco[i].length - 1);
		if(resultidreco[i][selector]){
			if(resultidreco[i].length > attr) {
				distribution[i] = (resultidreco[i][selector]);
				resultidreco[i][selector].selected = true;
			}
		}
		if(restidreco[i].length >= rest) {
			distribution_rest[i] = true;
		} else {
			distribution_rest[i] = false;
		}
	}
	...
	for (let i = 0; i < 18; i++){
		if(distribution[i] && distribution_rest[i]) {
			console.log(`${course_places[i]} : attractions - ${resultidreco[i].length}, restaurants - ${restidreco[i].length}`);
			chips += `<div onclick="sendText('${places[i]}')" id="chip${i + 1}" class="reco_button exists">${places[i]}</div>`;
			existing_course.push(`${course_places[i]}`);
		} else {
			chips += `<div id="chip${i + 1}" class="reco_button non_exist">${places[i]}</div>`;
		}
	}
	...
	if(data.question_one === "레저/스포츠") {
		title = `${quote.question_one} Course ${quote.question_two}!`;
	} else {
		title = `${quote.question_two} ${quote.question_one} Course!`;
	}
	let hashtag = `#${quote.question_one}course  #${quote.tag}  #${quote.question_three}`;
...
const reco_step_locale_view = async (data, rerun = false) => {
	...
	let title;
	if(data.question_one === "레저/레포츠") {
		title = `${quote.question_one} Course ${quote.question_two}!`;
	} else {
		title = `${quote.question_two} ${quote.question_one} Course!`;
	}
	let hashtag = `#${quote.question_one}course  #${quote.tag}  #${quote.question_three} #${quote.place}`;

	document.getElementById("reco_step_locale_acco_subject").innerHTML = `Recommend a good place to ${quote.acco}!`;
	document.getElementById("reco_step_locale_course_subject").innerHTML = `Would you like to go on a ${quote.tour} trip?`;
	...
	if(course.acco.length) {
		course.acco.forEach((word, i) => {
			...
			accommodations += `
			<div class="localeAcco" onclick="sendText('dormitory number ${i + 1}')">
		...
		accommodations = `There are no accommodations for your course.`
	}

	for(let i = 0; i < quote.days; i++){
		if(i === 0) results += `<div id="reco_step_locale_course_day1" class="locale_day day1"><div class="verticalLine"></div>`;
		else if (i === 1){
		...
		for(let j = 0; j < 5; j++){
			let number = j + 1 + i * 5;
			let word = course.course[number - 1];
			pdata.push({"lng" : word.mapx, "lat" : word.mapy, "parent" : "reco_locale"});
			results += `<div id="courselist${number}" class="course-list" onclick="sendText('${number}')">`;
			if([1, 4].indexOf(j) > -1) {
				results += `<div id="locale_circle${number}" class="locale-circle restaurant">
<div class="result-number"> ${number} </div>
</div>`;
				if(j === 1) {
					results += `<div class="course-name"> Day Course : <span>${word.type ? word.type : ""}</span></div>`;
				} else {
					results += `<div class="course-name"> Evening Course : <span>${word.type ? word.type : ""}</span></div>`;
				}
			} else {
				results += `<div id="locale_circle${number}" class="locale-circle">
<div class="result-number"> ${number} </div>
</div>
<div class="course-name"> Tourist Course ${days.shift()} : <span>${word.type ? word.type : ""}</span></div>`;
			}
			let imgURL = word.firstimage ? replaceimage(word.firstimage) : "./img/icon/noimage.png";
...
const makeCourse = async (data, rerun = false) => {
	...
	// 숙박업소 타입 지정
	let acco_smalltype = [];
	if(['one', 'three', 'six', 'eight'].indexOf(data.course_type) > 0) {
